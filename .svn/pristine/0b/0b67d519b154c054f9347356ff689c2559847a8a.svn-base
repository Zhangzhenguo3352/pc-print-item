(function(){
	...
	var colour = "#4285f4";
	var opacity = 0.3;
	var ripple_within_elements = [];
	var isrippleClassName = [];
	var noClassName = ['back', 'refresh', 'submit active', 'selectDown', 'Disabled','btn', 'submit', 'paymentAmount', 'selectOn_img', 'clickSet', 'active', 'selectOn', 'imgBox', 'add', 'delete less disabled', 'imgBox active', 'imgBoxAdd', 'flex', 'deleteFn']  // 那些class  不会有圆点
	var ripple_without_diameter = 90;
			...
			return function(){
				...
				switch($element.transition_phase){
					case 1:
						...
						setTimeout($element.next_transition, 0.15 * 1000); //now I know transitionend is better but it fires multiple times when multiple properties are animated, so this is simpler code and (imo) worth tiny delays
						break;
					case 2:
	...
	var click = function(event){
		...
		if(noClassName.indexOf(event.target.className) ==  -1) {
			...
			$element.style.opacity = opacity;
			if(ripple_within_elements.indexOf(touch.target.nodeName.toLowerCase()) > -1 || isrippleClassName.indexOf(event.target.className) > -1) {
				x = touch.offsetX;
				y = touch.offsetY;
			...
			setTimeout($element.next_transition, 10);
		}
